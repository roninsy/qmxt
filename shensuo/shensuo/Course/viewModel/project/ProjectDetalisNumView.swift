...
class ProjectDetalisNumView: UIView {
    ...
    let setpNumLab = UILabel.initSomeThing(title: "0", titleColor: .init(hex: "#333333"), font: .SemiboldFont(size: 24), ali: .center)
    ...
    var model : CourseStepListModel? = nil{
        didSet{
            if model != nil {
                ...
                let kllNum = model?.calorie?.intValue ?? 0
    ...
    var myModel : CourseDetalisModel? = nil{
        didSet{
            if myModel != nil {
                ...
                let protocolText = NSMutableAttributedString(string:  "\(getNumString(num: myModel?.totalCalorie?.doubleValue ?? 0))千卡")
                protocolText.bs_alignment = .center
