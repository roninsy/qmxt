...
-(void)drawRect:(CGRect)rect{
    ...
    CGContextSetRGBStrokeColor(context, 153 / 255.0, 153 / 255.0, 0.0, 1.0);//设置线的颜色，默认是黑色
    for (int i = 0; i <= _betweenNumber; i ++){
        ...
        if (i%_betweenNumber == 0){
            ...
            NSDictionary *attribute = @{NSFontAttributeName:TextRulerFont,NSForegroundColorAttributeName:[UIColor colorWithRed:222/255.0 green:222/255.0 blue:222/255.0 alpha:1]};
...
-(UITextField *)valueTF{
    if (!_valueTF) {
        _valueTF                          = [[UITextField alloc]initWithFrame:CGRectMake(self.bounds.size.width/2-150, 0,180, 40)];
...
-(UILabel *)unitLab{
    if (!_unitLab) {
        _unitLab = [[UILabel alloc]initWithFrame:CGRectMake( self.bounds.size.width/2+36 , 15, 40, 20)];
