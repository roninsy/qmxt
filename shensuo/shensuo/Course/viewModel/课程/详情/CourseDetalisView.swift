...
class CourseDetalisView: UIView {
    ...
    let topView = CourseDetailsHeadView()
    ...
    var myModel : CourseDetalisModel? = nil{
        didSet{
            ...
            self.tableview.jianJieCell.setpView.models = myModel!.courseStepList ?? (NSArray() as! [CourseStepListModel])
...
//        let cid = "212665065143558146"
//        let cid = "278797975134814212"
        ///有音频6600
        //        let cid = "295825493238824961"
//        let cid = "277967397179162658"
    ...
    ///完成支付回调
    @objc func payBack(notifi : Notification){
        self.payType = notifi.userInfo?["type"] as? String
        self.oid = notifi.userInfo?["oid"] as? String
    }
    
    ///完成支付回调
    @objc func payCallBack(notifi : Notification){
        if self.payType != nil && self.oid != nil{
            self.checkPay(cid: myModel?.id ?? "")
            self.payType = nil
            self.oid = nil
        }
    }
