...
open class LLCycleScrollView: UIView {
    ...
    open var imageArr : [UIImage] = []{
        didSet {
            totalItemsCount = imageArr.count
            if imageArr.count > 1 {
                collectionView.isScrollEnabled = true
            }else{
                collectionView.isScrollEnabled = false
...
extension LLCycleScrollView {
    ...
    ///
    /// - Returns: 下标-Index
    open func currentIndex() -> NSInteger {
...
extension LLCycleScrollView: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if isOnlyTitle && titles.count > 0{
            ...
            // 0==count 占位图
            if imagePaths.count == 0 && imageArr.count == 0 {
...
extension LLCycleScrollView: UIScrollViewDelegate {
    ...
    open func scrollViewWillBeginDragging(_ scrollView: UIScrollView) {
        self.selIndex = scrollView.contentOffset.x
    ...
    open func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        if imagePaths.count == 0 && imageArr.count == 0 { return }
        if imageArr.count > 0 {
            var index = Int(scrollView.contentOffset.x / UIScreen.main.bounds.size.width + (scrollView.contentOffset.x > self.selIndex ? 2 : 1))
            if index < 1 {
                index = 1
            }
            if index > imageArr.count {
                index = imageArr.count
            }
            delegate?.cycleScrollView?(self, scrollTo: NSInteger(index))
            return
        }
