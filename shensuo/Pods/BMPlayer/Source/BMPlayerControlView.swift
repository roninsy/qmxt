...
open class BMPlayerControlView: UIView {
    ...
    ///讲解按钮
    open var detalisBtn = UIButton()
    
    ///tv按钮
    open var tvBtn = UIView()
    
    ///镜像按钮
    open var jixiangBtn = UIButton()
    
    ///摄像头按钮
    open var cameraBtn = UIButton()
    ///礼物榜
    open var giftRankView = UIView()
    ///小节按钮
    open var stepNumView = UIView()
    
    ///当前天数
    open var dayView = UIView()
    open var dayLab = UILabel()
//    ///礼物显示按钮
//    open var giftBtn = UISwitch()
    
    ///格言文本
    open var geYanLab = UILabel()
    
    ///小节列表
    open var stepListView = UIView()
    
    ///小节详情视图
    open var stepDetalisView = UIView()
    
    ///小节点击按钮
    open var selBtn = UIButton()
...
//        progressView.tintColor = UIColor.init(red: 253/255.0, green: 128/255.0, blue: 36/255.0, alpha: 1)
        progressView.progressTintColor = UIColor.init(red: 255/255.0, green:255/255.0, blue: 255/255.0, alpha: 0.24)
        progressView.tintColor = UIColor.init(red: 253/255.0, green: 128/255.0, blue: 36/255.0, alpha: 1)
    ...
    open func controlViewAnimation(isShow: Bool) {
        ...
        if !isShow {
            self.stepListView.isHidden = true
            self.stepDetalisView.isHidden = true
        }
        ...
        UIView.animate(withDuration: 0.3, animations: {[weak self] in
          ...
          wSelf.bottomMaskView.alpha = alpha
            wSelf.geYanLab.alpha = alpha
            wSelf.stepListView.alpha = alpha
            wSelf.stepDetalisView.alpha = alpha
    ...
    func setupUIComponents() {
        ...
        topWrapperView.addSubview(detalisBtn)
        topWrapperView.addSubview(tvBtn)
        topWrapperView.addSubview(jixiangBtn)
        topWrapperView.addSubview(cameraBtn)
//        topWrapperView.addSubview(giftBtn)
        topWrapperView.addSubview(giftRankView)
        
        
        backButton.tag = BMPlayerControlView.ButtonType.back.rawValue
        backButton.setImage(BMImageResourcePath("white_close"), for: .normal)
//        backButton.addTarget(self, action: #selector(onButtonPressed(_:)), for: .touchUpInside)
        ...
        bottomWrapperView.addSubview(dayView)
        bottomWrapperView.addSubview(stepNumView)
        stepNumView.layer.cornerRadius = 6
        stepNumView.layer.masksToBounds = true
        dayView.layer.cornerRadius = 6
        dayView.layer.masksToBounds = true
        dayView.backgroundColor = .init(red: 0, green: 0, blue: 0, alpha: 0.5)
        
        dayLab.textColor = .white
        dayLab.font = .systemFont(ofSize: 12)
        dayLab.textAlignment = .center
        dayLab.adjustsFontSizeToFitWidth = true
        ...
        geYanLab.numberOfLines = 0
        geYanLab.textColor = .white
        geYanLab.font = UIFont.systemFont(ofSize: 18, weight: UIFont.Weight(rawValue: 0.23))
        geYanLab.textAlignment = .right
        mainMaskView.addSubview(geYanLab)
        geYanLab.snp.makeConstraints { make in
        ...
        self.mainMaskView.addSubview(selBtn)
        selBtn.snp.makeConstraints { make in
            make.edges.equalTo(stepNumView)
        }
        
        mainMaskView.addSubview(stepListView)
        mainMaskView.addSubview(stepDetalisView)
        stepListView.isHidden = true
        stepDetalisView.isHidden = true
    ...
    func addSnapKitConstraint() {
        ...
        detalisBtn.setTitle("讲解", for: .normal)
        detalisBtn.setTitleColor(.init(red: 0.2, green: 0.2, blue: 0.2, alpha: 1), for: .normal)
        detalisBtn.backgroundColor = UIColor.init(red: 1, green: 1, blue: 1, alpha: 1)
        detalisBtn.titleLabel?.font = .boldSystemFont(ofSize: 13)
        detalisBtn.layer.cornerRadius = 15
        detalisBtn.layer.masksToBounds = true
        ...
        tvBtn.snp.makeConstraints { make in
            ...
            make.centerY.equalTo(detalisBtn)
            make.right.equalTo(-250)
        }
        jixiangBtn.setImage(UIImage.init(named: "play_jingxiang_sel"), for: .selected)
        jixiangBtn.setImage(UIImage.init(named: "play_jingxiang"), for: .normal)
        jixiangBtn.snp.makeConstraints { make in
            make.height.width.equalTo(36)
            make.centerY.equalTo(detalisBtn)
            make.right.equalTo(-198)
        }
        cameraBtn.setImage(UIImage.init(named: "play_camera_sel"), for: .selected)
        cameraBtn.setImage(UIImage.init(named: "play_camera"), for: .normal)
...
//        giftBtn.onTintColor = .init(red: 253/255.0, green: 128/255.0, blue: 36/255.0, alpha: 1)
//        giftBtn.isOn = true
//        giftBtn.snp.makeConstraints { make in
//            make.width.equalTo(100)
//            make.height.equalTo(39)
//            make.centerY.equalTo(detalisBtn)
//            make.right.equalTo(detalisBtn.snp.left).offset(-8)
//        }
        ...
        giftRankView.snp.makeConstraints { make in
            ...
            make.height.equalTo(30)
            make.top.equalTo(21)
            make.left.equalTo(backButton.snp.right).offset(32)
        }
        ...
        dayView.snp.makeConstraints { make in
            make.width.equalTo(stepNumView)
            make.bottom.equalTo(stepNumView.snp.top).offset(7)
            make.left.equalTo(stepNumView)
